...
export function ContentSubmissionPanel() {
  return (
    <div className="bg-[#F9F6FF] shadow-sm  dark:bg-zinc-900/50 dark:border dark:border-zinc-500/30 rounded-2xl flex flex-col xs:flex-row xs:justify-around items-center p-4 w-full gap-3">
      {stats.map((item, idx) => (
        <div
          key={idx}
      ...
      flex-row items-center xs:items-start sm:items-center xs:justify-center 
        ...
        >
          ...
          <div className="flex text-start flex-col items-start sm:flex-row sm:items-center justify-center gap-1 ">
            <span className="xs:text-4xl  text-3xl font-bold text-black dark:text-zinc-200 leading-none">
              {item.count}
            </span>
